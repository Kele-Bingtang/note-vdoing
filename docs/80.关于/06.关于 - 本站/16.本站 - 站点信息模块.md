...
function readEachFileWords(excludeFiles = [''], cn, en) {
  ...
  filesList.forEach((item) => {
    ...
    wordsCount = len[0] + len[1];
    if (wordsCount >= 1000) {
      wordsCountStr = Math.round(wordsCount / 100) / 10 + 'k';
    }
    // fileMatterObj => {content:'剔除frontmatter后的文件内容字符串', data:{<frontmatter对象>}, ...}
    const fileMatterObj = matter(content, {});
    const matterData = fileMatterObj.data;
    filesListWords.push({ ...item, wordsCountStr, readingTime, ...matterData });
  });
  return filesListWords;
}
...
function readTime(len, cn = 300, en = 160) {
  let readingTime = len[0] / cn + len[1] / en;
  if (readingTime > 60 && readingTime < 60 * 24) {   // 大于一个小时，小于一天
    let hour = readingTime / 60;
    let minute = (readingTime - hour * 60);
    if (minute === 0) {
      return hour + 'h';
    }
    return hour + 'h' + minute + 'm';
  } else if (readingTime > 60 * 24) {      // 大于一天
    let day = readingTime / (60 * 24);
    let hour = (readingTime - day * 24 * 60) / 60;
    if (hour === 0) {
      return day + 'd';
    }
    return day + 'd' + hour + 'h';
  }
  return readingTime < 1 ? '1' : (readingTime * 10) / 10 + 'm';   // 取一位小数
}
